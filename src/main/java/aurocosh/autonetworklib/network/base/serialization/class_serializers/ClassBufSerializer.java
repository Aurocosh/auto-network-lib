package aurocosh.autonetworklib.network.base.serialization.class_serializers;
...
public class ClassBufSerializer {
    ...
    public ClassBufSerializer(Class clazz) {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            if (writer == null) {
                AutoNetworkLib.logger.error("Can't find buf writer for for field {} in class {}", field.getName(), clazz.getName());
                continue;
            }
            BufReader reader = readers[i];
            if (reader == null) {
                AutoNetworkLib.logger.error("Can't find buf reader for for field {} in class {}", field.getName(), clazz.getName());
                continue;
            }
